...
export const NewsLetterPage: FC<NewsLetterPageProps> = ({ className }) => {
  const [email, setEmail] = useState(extractEmailFromLocalstorage() ?? '');
  const { onSubmit: onSubmitUpdate } = useCreateNewsLetterSubscription();
    ...
    <>
      ...
      {subscribedLetter && data ? (
        <Container className={cx(styles.root, className)}>
          {isClientSide() && !localStorage.pianoUser ? (
            <div className={styles.heading}>
              <div>
                <div className={styles.block}>
                  <div>
                    <h1 className={styles.title}>
                      Real news, delivered straight to you
                    </h1>
                    <p className={styles.subTitle}>
                      Sign up to receive the latest stories, deals, news, data,
                      highlights insights you need to know for the industry
                    </p>
                  </div>
